...
with open("config.yaml", "r") as file:
    ...
    for key1 in config:
        ...
        for key2 in config[key1]:
            group.append(
                {"path": [key1, key2], "label": key2, "value": config[key1][key2]}
            )
...
with gr.Blocks(
    css="#log { padding:0; height: 0; overflow: hidden; } #log.display { padding: 10px 12px; height: auto; overflow: auto; } .gradio-container { max-width: none !important; }"
) as demo:
    with gr.Row():
        ...
        for component in components:
            with gr.Column():
                for item in component:
                    t = gr.Textbox(
                        label=item["label"], value=item["value"], interactive=True
                    )
